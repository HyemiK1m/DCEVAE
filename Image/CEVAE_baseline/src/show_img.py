...
def plot_results(model, args, epoch, whole_losses, x_recon_losses, l_recon_losses, u_kl_losses, cf_recon_losses):
    ...
    fixed_data, fixed_sens, fixed_rest, fixed_des = args.fixed_batch
    ...
    plt.subplot(122)
    rec_imgs = model.reconstruct_x(fixed_data.to(args.device), fixed_sens.to(args.device), fixed_rest.to(args.device), \
                                   fixed_des.to(args.device))
...
def test_results(args, model, fixed_data, fixed_sens, fixed_rest, fixed_des, test=True, epoch=0):
    ...
    rec_imgs = model.reconstruct_x(fixed_data, fixed_sens, fixed_rest, fixed_des)
    int_imgs = model.sampling_intervention(fixed_sens)
    # cond_imgs, a_cond = model.sampling_conditional(fixed_data)
    cf_imgs = model.sampling_counterfactual(fixed_data, fixed_sens, fixed_rest, fixed_des)
