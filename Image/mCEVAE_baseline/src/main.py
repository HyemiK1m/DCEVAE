...
parser.add_argument('--beta3', type=float, default=1, help='beta3') # mmd
parser.add_argument('--beta4', type=float, default=20, help='beta3') # mmd_a

parser.add_argument('--rep', type=int, default=1, help='beta3') # mmd_a
...
model = MCEVAE(args, sens_dim=sens_dim, rest_dim=rest_dim, des_dim=des_dim, u_dim=args.u_dim).to(args.device)
model = model.train()
...
args.save_path = os.path.join(result_path, "u_dim_{:d}_{:.1f}_{:.1f}_{:.1f}_{:.1f}"\
                              .format(args.u_dim, args.beta1, args.beta2, args.beta3, args.beta4))
